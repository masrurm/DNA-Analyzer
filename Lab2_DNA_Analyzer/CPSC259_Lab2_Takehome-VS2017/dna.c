...
void analyze_segments( char * sample_segment, char ** candidate_segments, int number_of_candidates )
{
    ...
    // goes through the file and checks for any perfect matches  
	for (i = 0; i < number_of_candidates; ++i) {

		if (strcmp(sample_segment, candidate_segments[i]) == 0) {
			printf("Candidate %d has a perfect match.\n", (i + 1));
			has_perfect_match++;
		}
	...
	// returns early from the function if any perfect matches are found
	...
	// calls calculate score if no perfect matches are found and prints every candidate's score
	for (i = 0; i < number_of_candidates; ++i) {

		printf("Candidate number %d matched with a best score of %d\n", (i + 1), calculate_score(sample_segment, candidate_segments[i]));
...
int calculate_score(char * sample_segment, char * candidate_segment)
{
	...
	char sample_segment_sub[4];
	char candidate_segment_sub[4];

	//For loop that shifts the sample
	for (k = 0; k <= candidate_length / 3 - sample_length/3; k++)
	{
		//Setting the temprary score to zero when we shift
		temp_score = 0;

		//For loop that switches between condons
		for (j = 0; j < sample_length_in_codons; j++)
		{
			//Creating the substring for condons
			for (i = 0; i < 3;i++) {
			sample_segment_sub[i] = sample_segment[i+3* (j)];
			candidate_segment_sub[i] = candidate_segment[i+3* (k + j)];
			}

		sample_segment_sub[3] = '\0';
		candidate_segment_sub[3] = '\0';

		//Checking if the condons are the same 
			if (sample_segment_sub[0] == candidate_segment_sub[0] &&
				sample_segment_sub[1] == candidate_segment_sub[1] &&
				sample_segment_sub[2] == candidate_segment_sub[2]) {
				temp_score += 10;
			}
			//Checking if they construct the same amino acid
			else if (strcmp(codon_names[get_codon_index(sample_segment_sub)],
				codon_names[get_codon_index(candidate_segment_sub)]) == 0) {
				temp_score += 5;

			}
			//If not, then we look at each individual character
			else {
				for (n = 0;n < 3;n++) {
					
					//Checking if the characters are the same
					if (sample_segment_sub[n] == candidate_segment_sub[n]) {
						temp_score += 2;
					}

					//Checking if they are a base pair
					if (is_base_pair(sample_segment_sub[n], candidate_segment_sub[n])) {
						temp_score += 1;
					}
		...
		//If the new score for the shift is bigger than any other score for this candidate, then change score
		if (temp_score > score)
			score = temp_score;
	}
